...
/*   Updated: 2024/06/30 13:33:47 by davgalle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_infile(t_string *infile, t_executor *exe, t_tools *tools)
{
	...
	if (ft_strlen(file) < 1)
		return (write(2, "No file introduced\n", 19), 1);
...
int	get_outfile(t_string *outfile, t_executor *exe)
{
	...
	str = ft_strtrim(outfile->str, ">");
	if (ft_strlen(str) < 1)
		return (write(2, "No file introduced\n", 19), 1);
	if (outfile->op == OP_OUTPUT_REDIRECT)
	{
		exe->outfile = open(str, O_CREAT | O_RDWR | O_TRUNC, 0644);
		if (exe->outfile < 0)
			exe->outfile = 1;
	}
	else
	{
		exe->outfile = open(str, O_WRONLY | O_CREAT | O_APPEND, 0644);
		if (exe->outfile < 0)
			exe->outfile = 1;
	}
	free(str);
